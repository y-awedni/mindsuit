 ...
 *
 * @Route("fiche/fournisseur/reglements")
 */
class ReglementsController extends Controller {
    ...
    public function getReglementsQbByParametres($em, $request, $fournisseur) {
    ...
    public function reglementsAction(Request $request, Fournisseur $fournisseur, $offset = 0, $limit = 10) {
        ...
        $items = $qb
                ...
                ->getQuery()
                ->getResult();
        return $this->render('fiche/fournisseur/reglements.html.twig', array(
                    'pagination' => $items,
    ...
    public function reglementsExportXlsAction(Request $request) {
        ...
        $filename = 'reglementsDeFournisseur' . $dateSys->format('d-m-Y_H:i:s') . '.xls';
        $phpExcelObject = $this->get('phpexcel')->createPHPExcelObject();
        $sheet = $phpExcelObject->setActiveSheetIndex(0);
        $sheet->setCellValue('A1', 'Liste des reglements de fournisseur ' . $fournisseur . ' ( ' . count($entities) . ' résultat(s) )');
        ...
        for ($j = 0; $j < count($entities); $j++) {
            ...
            $sheet->setCellValue('A' . $i, $vente['designation'])
                    ->setCellValue('B' . $i, $vente['typeDoc'])
