...
class ArticleController extends Controller {
    ...
    public function getTitreByParameteres($em, $request) {
        ...
        if ($request->query->get('categorie')) {
            $i > 0 ? $chaine .= ',' : $i++;
            $categorie = $em->getRepository('AppBundle:Categorie')->findOneById($request->query->get('categorie'));
            $chaine .= 'Catégorie : ' . $categorie;
        }
        if ($request->query->get('famille')) {
            $i > 0 ? $chaine .= ',' : $i++;
            $famille = $em->getRepository('AppBundle:Famille')->findOneById($request->query->get('famille'));
            $chaine .= 'Famille : ' . $famille;
        }
        if ($request->query->get('sousfamille')) {
            $i > 0 ? $chaine .= ',' : $i++;
            $sousfamille = $em->getRepository('AppBundle:Sousfamille')->findOneById($request->query->get('sousfamille'));
    ...
    public function indexAction(Request $request) {
        ...
        $sousfamilles = $em->getRepository('AppBundle:Sousfamille')->findAll();
    ...
    public function newAction(Request $request) {
        ...
        $form->add('fournisseur', EntityType::class, ['attr' => ['class' => 'selectpicker', 'data-live-search' => true, 'title' => 'Chercher et sélectionner'], 'class' => 'AppBundle:Fournisseur']);
    ...
    public function customNewAction(Request $request) {
        $article = new Article();
        $form = $this->createForm('AppBundle\Form\Custom\ArticleType', $article);
        $form->handleRequest($request);
