...
class FactureAvoirController extends Controller {
    ...
    public function getTitreByParameteres($em, $request) {
        ...
        if ($request->query->get('facture')) {
            $i > 0 ? $chaine .= ',' : $i++;
            $facture = $em->getRepository('AppBundle:Facture')->findOneById($request->query->get('facture'));
    ...
    public function getQbByParametres($em, $request) {
        $qb = $em->getRepository('AppBundle:FactureAvoir')->createQueryBuilder('a');
        $qb->join('a.facture', 'f');
    ...
    public function factureNewAction(Request $request, Facture $facture) {
        ...
        $form = $this->createForm('AppBundle\Form\FactureAvoirType', $factureAvoir);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            foreach ($factureAvoir->getLigneFactureAvoirs() as $key => $ligne) {
                ...
                $qb = $em->getRepository('AppBundle:LigneFactureAvoir')->createQueryBuilder('lfa');
    ...
    public function editAction(Request $request, FactureAvoir $factureAvoir) {
        ...
        $editForm = $this->createForm('AppBundle\Form\FactureAvoirType', $factureAvoir);
