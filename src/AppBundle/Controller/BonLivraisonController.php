...
class BonLivraisonController extends Controller {
    ...
    public function getQbByParametres($em, $request) {
        $qb = $em->getRepository('AppBundle:BonLivraison')->createQueryBuilder('a');
    ...
    public function editAction(Request $request, BonLivraison $bonLivraison) {
        ...
        $editForm = $this->createForm('AppBundle\Form\BonLivraisonType', $bonLivraison, array(
    ...
    public function reglementsAction(Request $request, BonLivraison $bonlivraison) {
        ...
        $form = $this->createForm('AppBundle\Form\LigneReglementBonLivraisonType', $ligneReglement);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
        ...
        $ligneReglements = $em->getRepository('AppBundle:LigneReglementBonLivraison')->findByBonLivraison($bonlivraison);
    ...
    public function reglementDeleteAction($id, $idLigneReglement) {
        $em = $this->getDoctrine()->getManager();
        $ligneReglement = $em->getRepository('AppBundle:LigneReglementBonLivraison')->find($idLigneReglement);
    ...
    public function convertirBlsEnFactureAction(Request $request) {
        ...
        if (count($bls) > 0) {
            ...
            $i = 0;
            foreach ($bls as $bl) {
                $bonlivraison = $em->getRepository('AppBundle:BonLivraison')->findOneByCode($bl);
                if (!$bonlivraison->getConverted()) {
                    foreach ($bonlivraison->getLigneBonLivraisons() as $ligne) {
                        $lf = new LigneFacture();
                        $lf->setArticle($ligne->getArticle());
    ...
    public function deleteAction(BonLivraison $bonlivraison) {
        ...
        foreach ($ligneBonLivraisons as $ligne) {
            ...
            $article = $em->getRepository('AppBundle:Article')->findOneById($ligne->getArticle()->getId());
        ...
        $ligne_reglement_bon_livraisons = $em->getRepository('AppBundle:LigneReglementBonLivraison')->findByBonLivraison($bonlivraison);
        foreach ($ligne_reglement_bon_livraisons as $ligne) {
            $this->deleteFunction($em, $ligne); //suppression des reglements
        }
        $stocks = $em->getRepository('AppBundle:Stock')->findByDesignation($bonlivraison->getCode());
