...
class NotificationController extends Controller {
    ...
    public function indexAction(Request $request) {
        ...
        $qb = $em->getRepository('AppBundle:Notification')->createQueryBuilder('a');
        //filter
    ...
    public function newsAction() {
        $em = $this->getDoctrine()->getManager();
        $notifications = $notifications = $em->getRepository('AppBundle:Notification')->findBy([], ['id' => 'DESC'], '8');
        $countAlerts = count($em->getRepository('AppBundle:Notification')->findByVu(false));
    ...
    public function vuAllAction() {
        $em = $this->getDoctrine()->getManager();
        $notifications = $em->getRepository('AppBundle:Notification')->findByVu(false);
    ...
    public function changeNotificationVuAction(Request $request) {
        ...
        $em = $this->getDoctrine()->getManager();
        $notification = $em->getRepository('AppBundle:Notification')->find($request->request->get('id'));
        $notification->setVu(true);
        $em->flush($notification);
        $response = array(
            'success' => true
