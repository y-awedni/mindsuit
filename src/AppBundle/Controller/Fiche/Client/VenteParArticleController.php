...
namespace AppBundle\Controller\Fiche\Client;
 ...
 *
 * @Route("fiche/client/vente/article")
 */
class VenteParArticleController extends Controller {

    public function getTitreVenteParArticleByParameteres($em, $request) {
        $chaine = '(';
        $i = 0;
    ...
    public function getVenteParArticleQbByParametres($em, $request, $client) {
    ...
    public function venteParArticleAction(Request $request, Client $client, $offset = 0, $limit = 10) {
        ...
        $qb = $this->getVenteParArticleQbByParametres($em, $request, $client->getId());
        ...
        $items = $qb
                ...
                ->getQuery()
                ->getResult();
        return $this->render('fiche/client/venteParArticle.html.twig', array(
                    'pagination' => $items,
    ...
    public function venteParArticleExportXlsAction(Request $request) {
        $em = $this->getDoctrine()->getManager();
        $qb = $this->getVenteParArticleQbByParametres($em, $request, $request->query->get('client'))
                ->addGroupBy('a.article');

        $client = $em->getRepository('AppBundle:Client')->find($request->query->get('client'));
        $titre = $this->getTitreVenteParArticleByParameteres($em, $request);
        $entities = $qb->getQuery()->getResult();
        $dateSys = new \DateTime();
        $filename = 'venteParArticle' . $dateSys->format('d-m-Y_H:i:s') . '.xls';
        $phpExcelObject = $this->get('phpexcel')->createPHPExcelObject();
        $sheet = $phpExcelObject->setActiveSheetIndex(0);
        $sheet->setCellValue('A1', 'Liste des ventes par article de client ' . $client . ' ( ' . count($entities) . ' résultat(s) )');
