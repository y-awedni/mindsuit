...
class FactureRepository extends EntityRepository {
    ...
    public function findAllByYear($year) {
        ...
        return $this->getEntityManager()
                        ->createQuery(
                                "SELECT count(f.id) FROM AppBundle:Facture f WHERE f.createdAt BETWEEN '" . $min . "' AND '" . $max . "'"
                        )
    ...
    public function findAllCount() {
        return $this->getEntityManager()
                        ->createQuery(
                                "SELECT count(f.id) FROM AppBundle:Facture f"
                        )
                        ->getSingleScalarResult();
    }
    ...
    public function findAllRevenusByInterval($d1,$d2) {
        ...
        return $this->getEntityManager()
                        ->createQuery(
                                "SELECT f FROM AppBundle:Facture f WHERE f.dateCreation BETWEEN '" . $dateDebut . "' AND '" . $dateFin . "' ORDER BY f.dateCreation ASC"
                        )
                        ->getResult();
    }
    ...
    public function findAllEcheanceByDateSys($days){
        ...
        return $this->getEntityManager()
                        ->createQuery(
                                "SELECT count(f.id) FROM AppBundle:Facture f WHERE f.regle<f.total AND f.dateEcheance ='" . date_format($sys_date, $format) . "'"
                        )
                        ->getSingleScalarResult();
    }
    ...
    public function findAllTotalAvoirRemboursement($factureId){
        ...
        return $this->getEntityManager()
                        ->createQuery(
                                "SELECT sum(lfa.ttc) FROM AppBundle:LigneFactureAvoir lfa "
