...
class BonReceptionType extends AbstractType {
    ...
    public function buildForm(FormBuilderInterface $builder, array $options) {
        $builder
                ...
                ->add('bonCommande', EntityType::class, 
                        ['attr' => [
                            'required'=>false,
                            'class' => 'selectpicker',
                            ...
                            'class' => 'AppBundle:BonCommandeFrs'
                        ])
        ...
        $builder->add('ligneBonReceptions', CollectionType::class, array(
            'entry_type' => LigneBonReceptionType::class,
            'allow_add' => true,
            'allow_delete' => true,
            'prototype' => true,
            'mapped' => true,
        ...
        $builder->addEventListener(FormEvents::PRE_SET_DATA, function (FormEvent $event) {
            $bonReception = $event->getData();
            $form = $event->getForm();

            if (!$bonReception) {
                return;
            }
            if (!$bonReception->getId() or $bonReception->getRegle() == 0) {
        ...
        ->addEventListener(FormEvents::PRE_SUBMIT, function (FormEvent $event) {
                    ...
                    foreach ($entity['ligneBonReceptions'] as $key => $ligne) {
                        if ($entity['ligneBonReceptions'][$key]['article'] === "") {
                            unset($entity['ligneBonReceptions'][$key]);
                        }
                    }
    ...
    public function configureOptions(OptionsResolver $resolver) {
        $resolver->setDefaults(array(
            'data_class' => 'AppBundle\Entity\BonReception'
        ));
    }
